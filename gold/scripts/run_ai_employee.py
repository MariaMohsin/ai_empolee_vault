...
class AIEmployeeScheduler:
    ...
    def run_personal_tasks(self):
        """Auto-process personal domain tasks from Personal/Inbox/."""
        try:
            script = self.root / ".claude" / "skills" / "personal-tasks" / "scripts" / "personal_tasks.py"
            if not script.exists():
                return {"success": False, "processed": 0, "note": "personal_tasks.py not found"}
            result = subprocess.run(
                [sys.executable, str(script), "--process"],
                capture_output=True, text=True, timeout=60,
                env={**os.environ}
            )
            self.log("PERSONAL", (result.stdout + result.stderr).strip()[-200:] or "no output")
            processed = 0
            ...
            return {"success": result.returncode == 0, "processed": processed}
        except Exception as e:
            self.log("ERROR", f"Personal tasks failed: {e}")
            return {"success": False, "processed": 0}

    def run_ralph_loop(self):
        """Run Ralph Wiggum autonomous loop on Needs_Action tasks."""
        try:
            ralph_script = self.root / ".claude" / "skills" / "ralph-wiggum-loop" / "scripts" / "ralph_loop.py"
            if not ralph_script.exists():
                return {"success": False, "done": 0, "failed": 0, "pending": 0, "note": "ralph_loop.py not found"}

            import subprocess
            result = subprocess.run(
                [sys.executable, str(ralph_script), "--once"],
                capture_output=True, text=True, timeout=120,
                env={**os.environ}
            )
            self.log("RALPH", (result.stdout + result.stderr).strip()[-300:] or "no output")

            # Parse stats from output if present
            stats = {"success": result.returncode == 0, "done": 0, "failed": 0, "pending": 0, "retry": 0}
            for line in (result.stdout + result.stderr).splitlines():
                if "Stats:" in line and "done" in line:
                    try:
                        import re
                        for key in ["done", "failed", "pending", "retry"]:
            ...
            self.log("ERROR", f"Ralph loop failed: {e}")
            return {"success": False, "done": 0, "failed": 0, "pending": 0}
    ...
    def execute_work_cycle(self):
        ...
        try:
            ...
            # Step 3: Ralph Wiggum Loop — autonomous multi-step task execution
            self.log("INFO", "Step 3: Running Ralph Wiggum loop (Needs_Action tasks)...")
            ralph_result = self.run_ralph_loop()
            self.log("INFO", f"  Ralph loop: {ralph_result.get('done', 0)} done, {ralph_result.get('failed', 0)} failed, {ralph_result.get('pending', 0)} pending")

            # Step 4: Personal Tasks — process personal domain inbox
            self.log("INFO", "Step 4: Processing personal tasks...")
            personal_result = self.run_personal_tasks()
            self.log("INFO", f"  Personal tasks: {personal_result.get('processed', 0)} processed")

            # Step 5: MCP Executor — execute approved actions in Needs_Approval
            self.log("INFO", "Step 5: Running MCP executor (approved actions)...")
